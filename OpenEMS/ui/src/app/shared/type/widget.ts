...
export class Widgets {
  public static parseWidgets(edge: Edge, config: EdgeConfig): Widgets {
    ...
    for (let nature of Object.values(WidgetNature).filter(
      (v) => typeof v === 'string'
    )) {
      for (let componentId of config.getComponentIdsImplementingNature(
        nature.toString()
      )) {
        if (
    ...
    for (let factory of Object.values(WidgetFactory).filter(
      (v) => typeof v === 'string'
    )) {
      for (let componentId of config.getComponentIdsByFactory(
        factory.toString()
      )) {
        if (config.getComponent(componentId).isEnabled) {
    ...
    list.sort((w1, w2) => {
      if (
        ...
        let outputChannelAddress1: string | string[] =
          config.getComponentProperties(w1.componentId)['outputChannelAddress'];
        ...
        let outputChannelAddress2: string | string[] =
          config.getComponentProperties(w2.componentId)['outputChannelAddress'];
