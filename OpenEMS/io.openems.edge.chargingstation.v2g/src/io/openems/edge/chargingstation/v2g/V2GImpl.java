...
public class V2GImpl extends AbstractOpenemsComponent implements V2G, OpenemsComponent, EventHandler  {
	...
	public void getData() throws Exception {
		URL obj = new URL(config.ipAddress());
		...
		if (responseCode == HttpURLConnection.HTTP_OK) {
			...
			if (JsonParser.parseString(response.toString()).isJsonObject()) {
				...
				this._setGridL1Volts(jo.get("gridvoltage").getAsDouble());
				this._setStatus(jo.get("status").getAsInt());
				this._setPowerUnitTemperature(jo.get("temperature").getAsDouble());
			} else {
